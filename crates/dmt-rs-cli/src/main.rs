//! dmt-rs CLI - High-performance cross-database migration tool.
...
#[derive(Parser)]
#[command(name = "dmt-rs")]
#[command(about = "dmt-rs: high-performance cross-database migration")]
#[command(version)]
struct Cli {
...
enum Commands {
    ...
    ///
    /// State is automatically stored in the target PostgreSQL database (_dmt_rs schema):
    ...
    ///
    /// State is stored in the target PostgreSQL database (_dmt_rs schema).
    Resume {
        /// Override source schema
        #[arg(long)]
        source_schema: Option<String>,
