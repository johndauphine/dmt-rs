//! CLI integration tests for dmt-rs.
//!
...
/// Get a command for the dmt-rs binary.
#[allow(deprecated)]
fn cmd() -> Command {
    Command::cargo_bin("dmt-rs").unwrap()
}
...
fn test_version_flag() {
    cmd()
        ...
        .assert()
        .success()
        // Verify both program name and semver version format (e.g., "dmt-rs 0.8.0")
        .stdout(predicate::str::is_match(r"dmt-rs \d+\.\d+\.\d+").unwrap());
}
...
// (_dmt_rs schema) instead of a file
