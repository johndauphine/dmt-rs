            ...
            SqlValue::CompressedText {
                ...
                match lz4_flex::decompress_size_prepended(compressed) {
                    Ok(decompressed) => {
                        ...
                        String::from_utf8(decompressed).ok().map(Cow::Owned)
                    }
                    Err(_) => None,
                }
            }
...
mod tests {
    ...
    fn test_decompress_text_returns_none_for_non_text() {
        ...
        assert!(SqlValue::Null(SqlNullType::String)
            .decompress_text()
            .is_none());
        assert!(SqlValue::bytes_owned(vec![1, 2, 3])
            .decompress_text()
            .is_none());
    }
    ...
    fn test_compression_threshold_boundary() {
        ...
        assert_eq!(result_at.decompress_text().unwrap().as_ref(), at_threshold);
        assert_eq!(
