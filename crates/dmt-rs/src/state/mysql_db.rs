...
//!
//! Stores migration state in MySQL tables within the `_dmt_rs` database.
...
impl MysqlStateBackend {
    ...
    pub async fn load_latest(&self, config_hash: &str) -> Result<Option<MigrationState>> {
        ...
        for row in rows {
            ...
            let rows_total: i64 = row
                .get::<Option<i64>, _>("rows_total")
                .flatten()
                .unwrap_or(0);
            let rows_transferred: i64 = row
                .get::<Option<i64>, _>("rows_transferred")
                .flatten()
                .unwrap_or(0);
            let rows_skipped: i64 = row
                .get::<Option<i64>, _>("rows_skipped")
                .flatten()
                .unwrap_or(0);
