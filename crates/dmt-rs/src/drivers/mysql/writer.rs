...
impl MysqlWriter {
    pub async fn new(
        ...
        // Per-connection initialization. Runs on each pooled connection when it's
        // first created. Anything here MUST work on every targeted MySQL flavor/
        // version — a failure bricks the entire pool.
        let init_stmts: Vec<String> = vec!["SET NAMES utf8mb4".to_string()];
