...
### High
- Dual architectures are active and cause duplication plus conversion glue. Evidence: legacy traits in `crates/dmt-rs/src/source/mod.rs:70` and `crates/dmt-rs/src/target/mod.rs:51`, new traits in `crates/dmt-rs/src/core/traits.rs:69`, both exported in `crates/dmt-rs/src/lib.rs:60`; bridging conversions for MySQL in `crates/dmt-rs/src/orchestrator/pools.rs:44` and `crates/dmt-rs/src/drivers/mysql/reader.rs:15`. Action: refactor transfer/orchestrator to use new core/drivers APIs and delete legacy modules.
- Pipeline module appears unused. Evidence: only re-exported in `crates/dmt-rs/src/lib.rs:60`, implemented in `crates/dmt-rs/src/pipeline/mod.rs:1` with no internal references. Action: either adopt pipeline as the transfer abstraction or delete it to reduce surface area.
- Duplicate transfer primitives. Evidence: `DateFilter` and `TransferJob` are defined in both `crates/dmt-rs/src/transfer/mod.rs:22` and `crates/dmt-rs/src/pipeline/job.rs:18`. Action: keep one set and delete the other.
- Duplicate SQL value representations. Evidence: `SqlValue` and `SqlNullType` exist in `crates/dmt-rs/src/core/value.rs:16` and again in `crates/dmt-rs/src/target/mod.rs:189`, with conversion glue in `crates/dmt-rs/src/orchestrator/pools.rs:44`. Action: standardize on core::value types and remove legacy SqlValue types.
- Type mapping is duplicated between old typemap and new dialect mappers. Evidence: legacy mapping in `crates/dmt-rs/src/typemap/mod.rs:1` is still used by the target code in `crates/dmt-rs/src/target/mod.rs:25`, while new canonical mappers live in `crates/dmt-rs/src/dialect/mod.rs:1`. Action: keep dialect mappers and remove legacy typemap.
- Driver catalog entry points are defined but unused. Evidence: `crates/dmt-rs/src/orchestrator/mod.rs:418` defines create_source_reader/create_target_writer but no call sites exist. Action: wire these into the transfer path (preferred) and remove unused legacy factories.
- NoOp state backend is unused. Evidence: `crates/dmt-rs/src/state/noop.rs:1` and `crates/dmt-rs/src/state/mod.rs:61` define NoOp, but `crates/dmt-rs/src/orchestrator/pools.rs:562` only constructs Postgres/MSSQL/MySQL backends. Action: delete NoOpStateBackend.

### Medium
- SourcePool/TargetPool traits and enums are part of the legacy stack. Evidence: traits in `crates/dmt-rs/src/source/mod.rs:70` and `crates/dmt-rs/src/target/mod.rs:51` vs enum wrappers in `crates/dmt-rs/src/orchestrator/pools.rs:162`. Action: remove these once the transfer path uses `core::traits::{SourceReader, TargetWriter}`.
- Config flags are partially unused in the transfer engine. Evidence: `TransferConfig` is always created with `use_copy_binary: true` and `use_direct_copy: true` in `crates/dmt-rs/src/orchestrator/mod.rs:920`, ignoring `migration.use_binary_copy`. Action: wire `migration.use_binary_copy` into the new transfer path or delete the config knob.
- Unused dependencies inflate build surface. Evidence: `enum_dispatch` (unused, `crates/dmt-rs/Cargo.toml:26`), `parking_lot` (unused, `crates/dmt-rs/Cargo.toml:64`), `rayon` (unused, `crates/dmt-rs/Cargo.toml:65`), `md-5` (unused, `crates/dmt-rs/Cargo.toml:76`), `anyhow` (unused in CLI, `crates/dmt-rs-cli/Cargo.toml:31`), and workspace `deadpool` (unused, `Cargo.toml:79`). Action: delete these dependencies and update Cargo.lock.
- Public API surface is very broad due to re-exports. Evidence: `crates/dmt-rs/src/lib.rs:46` through `crates/dmt-rs/src/lib.rs:76` re-exports both legacy and new APIs. Action: narrow exports to new architecture only.

### Low
- Dead fields left for future use. Evidence: `WriteJob.partition_id` in `crates/dmt-rs/src/transfer/mod.rs:270`, `PgUpsertWriter.copy_buffer_rows` in `crates/dmt-rs/src/target/mod.rs:1570`, `MysqlReader.database` in `crates/dmt-rs/src/drivers/mysql/reader.rs:23`. Action: remove these fields or implement the intended functionality.
- Bench-only module adds maintenance surface. Evidence: `crates/dmt-rs/src/source/bench_copy.rs:1` is an ignored benchmark with local credentials. Action: move to `benches/` or delete if not actively used.
...
1) Refactor transfer/orchestrator to use the new core/drivers path end-to-end.
   - Use `DriverCatalog::create_reader/create_writer` in `crates/dmt-rs/src/orchestrator/mod.rs:418` and feed `SourceReaderImpl`/`TargetWriterImpl` into the transfer engine.
   - Replace `SourcePoolImpl`/`TargetPoolImpl` and delete `crates/dmt-rs/src/orchestrator/pools.rs` once migration is complete.
2) Remove legacy modules once the new path is wired.
   - Delete `crates/dmt-rs/src/source/`.
   - Delete `crates/dmt-rs/src/target/`.
   - Delete `crates/dmt-rs/src/typemap/`.
   - Remove legacy re-exports from `crates/dmt-rs/src/lib.rs:46`.
3) Consolidate transfer primitives.
   - Keep a single `DateFilter` and `TransferJob` definition.
   - If pipeline remains unused, remove `crates/dmt-rs/src/pipeline/`; otherwise migrate the transfer engine to the pipeline and delete the legacy `TransferJob`/`DateFilter` in `crates/dmt-rs/src/transfer/mod.rs:22`.
4) Standardize on `core::value::SqlValue` and remove conversions.
   - Delete legacy `SqlValue`/`SqlNullType` in `crates/dmt-rs/src/target/mod.rs:189`.
   - Remove conversion glue in `crates/dmt-rs/src/orchestrator/pools.rs:44` and `crates/dmt-rs/src/drivers/mysql/reader.rs:15`.
5) Update state backends to work with new drivers.
   - Replace `MssqlTargetPool`/`PgPool` usage in `crates/dmt-rs/src/state/*` with new driver connection pools or a thin new `state` client built on `drivers/common` utilities.
   - Remove `NoOpStateBackend` (`crates/dmt-rs/src/state/noop.rs:1`).
6) Clean dependencies and public API surface.
   ...
   - Narrow `crates/dmt-rs/src/lib.rs:46` to new architecture exports only.
...
2) Choose the transfer abstraction
   - Option A (recommended): keep `crates/dmt-rs/src/transfer/` but port it to `core::traits::{SourceReader, TargetWriter}`.
...
4) Wire orchestrator to new engine
   - Update `crates/dmt-rs/src/orchestrator/mod.rs:418` to call `DriverCatalog::create_reader/create_writer`.
...
6) Remove legacy modules
   - Delete `crates/dmt-rs/src/source/`, `crates/dmt-rs/src/target/`, `crates/dmt-rs/src/typemap/`, and `crates/dmt-rs/src/orchestrator/pools.rs`.
...
7) Clean public API and dependencies
   - Narrow `crates/dmt-rs/src/lib.rs:46` exports to new architecture only.
...
1) `TransferEngine::execute` (`crates/dmt-rs/src/transfer/mod.rs:349`)
...
8) `quote_*` and `qualify_*` helpers (`crates/dmt-rs/src/transfer/mod.rs:1740`)
...
- Run core CLI flows: `target/release/dmt-rs -c config.yaml run` and `target/release/dmt-rs init`
- If TUI remains enabled: `cargo run --features tui --bin dmt-rs -- tui`
